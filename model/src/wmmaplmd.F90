...
#define METHOD "Initialize"
      subroutine Initialize ( gc, impState, expState, extClock, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |        T. J. Campbell, NRL        |
...
!
      rc = ESMF_SUCCESS
!ALT      call ESMF_VMWtime(wstime)
      call ESMF_GridCompGet(gc, name=cname, __RC__)
...
!/
!
! part of InitializeP1
! -------------------------------------------------------------------- /
! 1.  Initialization necessary for driver
!
! 1.a Set global flag indicating that model is an ESMF Component
!
!ALT      is_esmf_component = .true.
!ALT the next 2 line should not be needed, but I'll leave them for now
...
!
      ...
      call ESMF_VMGet(vm, petCount=npet, localPet=lpet, &
        mpiCommunicator=mpiComm, __RC__)
      nmproc = npet
      improc = lpet + 1
      nmpscr = 1
!ALT: root PE write diag message
...
!
!ALT: we need a strategy to tell WW3 how long to run
! the existing one is bad! It keys off the stop time of the clock
! we could use current time as start, and increment it by clock's
! time step to set end time
 
      call ESMF_ClockGet(extClock, currTime=ttmp, __RC__)
      call ESMF_TimeGet(ttmp, yy=yy,mm=mm,dd=dd,h=h,m=m,s=s, __RC__)
      stime(1) = 10000*yy + 100*mm + dd
      stime(2) = 10000*h  + 100*m  + s
 
      call ESMF_ClockGet(extClock, timeStep=etstep, __RC__)
      ttmp = ttmp + etstep
      call ESMF_TimeGet(ttmp, yy=yy,mm=mm,dd=dd,h=h,m=m,s=s, __RC__)
...
!
!ALT: I will leave this in place, but this is a bottleneck at large CPU count
...
!
!ALT      call ESMF_ClockGet(extClock, timeStep=etstep, rc=rc)
!ALT      if (ESMF_LogFoundError(rc, PASSTHRU)) return
!
! 2.c Trap unsupported CPL input forcing settings
!
 
!ALT: check with AD how to handle this
      if ( any(inpmap.lt.0) ) then
        ...
        allocate (cplmap(nrgrd,jfirst:8), __STAT__)
        jmod = minval(inpmap)
        cplmap = inpmap
...
!
!      rc = ESMF_SUCCESS
!ALT      if (verbosity.gt.0) call ESMF_LogWrite(trim(cname)// &
!ALT        ': leaving InitializeP1', ESMF_LOGMSG_INFO)
!
! -------------------------------------------------------------------- /
! Formats
!
...
!/ End of InitializeP1 ----------------------------------------------- /
!/
!ALT      end subroutine InitializeP1
!/ ------------------------------------------------------------------- /
#undef METHOD
#define METHOD "InitializeP3"
!ALT      subroutine InitializeP3 ( gc, impState, expState, extClock, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |        T. J. Campbell, NRL        |
...
!     ----------------------------------------------------------------
!
!ALT      character(ESMF_MAXSTR) :: cname
!ALT      integer :: i1, i2, i3, i, n
!ALT      logical :: isConnected
!
! -------------------------------------------------------------------- /
! Prep
!
!ALT      rc = ESMF_SUCCESS
!ALT      call ESMF_VMWtime(wstime)
!ALT      call ESMF_GridCompGet(gc, name=cname, rc=rc)
!ALT      if (ESMF_LogFoundError(rc, PASSTHRU)) return
!ALT      if (verbosity.gt.0) call ESMF_LogWrite(trim(cname)// &
!ALT        ': entered InitializeP3', ESMF_LOGMSG_INFO)
 
!ALT I thin we can safely skip 1,2 (pretty much to the end of P3)
!
! Call GenericInitialize
! ----------------------
      call MAPL_GenericInitialize(GC, impState, expState, extCLOCK, __RC__)
 
      call DataInitialize(gc, impState, expState, extClock, __RC__)
...
!
      do i = 1,numExpFields
        ...
        if (ESMF_LogFoundError(rc, PASSTHRU)) return
!geos!  expFieldActive(i) = isConnected .or. realizeAllExport
...
!
      if ( (GTYPE.eq.RLGTYPE).or.(GTYPE.eq.CLGTYPE) ) then
         ...
         call CreateExpGrid( gc, rc=rc )
         if (ESMF_LogFoundError(rc, PASSTHRU)) return
      elseif (GTYPE.eq.UNGTYPE) then
         write(msg,'(a)') trim(cname)// &
           ': Creating export mesh for Unstructured Mode'
         call ESMF_LogWrite(trim(msg), ESMF_LOGMSG_INFO)
         call CreateExpMesh( gc, rc=rc )
         if (ESMF_LogFoundError(rc, PASSTHRU)) return
      endif
!
...
!
#ifdef USE_W3OUTG_FOR_EXPORT_for_now_this_is_disabled
...
!ALT      rc = ESMF_SUCCESS
!ALT      if (verbosity.gt.0) call ESMF_LogWrite(trim(cname)// &
!ALT        ': leaving InitializeP3', ESMF_LOGMSG_INFO)
!/
!/ End of InitializeP3 ----------------------------------------------- /
!/
#endif !if 0
!ALT      end subroutine InitializeP3
...
!      Name            Type  Module   Description
!     ----------------------------------------------------------------
!      GetImport       Subr. WMMAPLMD Wave model get import fields
!      SetExport       Subr. WMMAPLMD Wave model set export fields
!     ----------------------------------------------------------------
!
!  5. Called by :
!
...
!
      call GetImport(gc, import, clock, __RC__)
!
...
!
!ALT      call SetExport(gc, export, __RC__)
!
...
!
      ...
      allocate (tend(2,nrgrd), __STAT__)
!
! -------------------------------------------------------------------- /
...
!
! 1.d Set end time of this advance (current+time_step)
!
      call ESMF_ClockGet(clock, timeStep=timestep, __RC__)
      stopTime = currTime + timestep
      call ESMF_TimeGet(stopTime, yy=yy,mm=mm,dd=dd,h=h,m=m,s=s, __RC__)
      do imod = 1,nrgrd
...
!
      call SetExport(gc, export, __RC__)
 !ALT old form     call SetExport(gc, __RC__)
...
#define METHOD "GetImport"
      subroutine GetImport ( gc, import, clock, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |        T. J. Campbell, NRL        |
...
#endif
      real, pointer :: uwind(:,:) => null()
      real, pointer :: vwind(:,:) => null()
...
!ALT      call ESMF_VMWtime(wstime)
      ...
      call NUOPC_ModelGet(gc, importState=dumpState, rc=rc)
      if (ESMF_LogFoundError(rc, PASSTHRU)) return
...
!
      ...
      ! j = 2
      ! vars: cxn, cyn
      ! deal also with: cx0, cy0
!
! -------------------------------------------------------------------- /
! Winds
!
      j = 3
      ! get pointer(s) to import
      call MAPL_GetPointer(import, uwind, 'U10M', __RC__)
      call MAPL_GetPointer(import, vwind, 'V10M', __RC__)
      ...
      call ESMF_StateGet(import, 'U10M', field, __RC__)
      call FieldGather( field, nx, ny, wxn, rc=rc )
      call ESMF_StateGet(import, 'V10M', field, __RC__)
...
!      wyn = vwind
      ...
      do imod = 1,nrgrd
         ...
         if ( jmod.lt.0 .and. jmod.ne.-999 ) then
            call wmupd2( imod, j, jmod, status )
            _VERIFY(status)
         endif
      enddo
...
!ALT      call FieldGather( impField(i1), nx, ny, icei, rc=rc )
      call MAPL_GetPointer(import, fraci, 'FRACI', __RC__)
!      icei = fraci
      call ESMF_StateGet(import, 'FRACI', field, __RC__)
      call FieldGather( field, nx, ny, icei, __RC__ )
...
#define METHOD "SetExport"
      subroutine SetExport ( gc, export, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |        T. J. Campbell, NRL        |
...
!/
      ...
      logical :: flpart = .false., floutg = .true., floutg2 = .false.
      integer :: status
      integer :: i1, in, j1, jn
...
!-----------------------------------------------------------
      call ESMF_GridCompGet(gc, vm=vm, __RC__)
      call ESMF_VMGet(vm, localPet=mype, petCount=npes, mpiCommunicator=comm, __RC__)
      amIRoot = (mype==MAPL_Root)
      if (amIRoot) then
         allocate(nSeaLocal(0:npes-1), __STAT__)
      else
         allocate(nSeaLocal(0), __STAT__)
      end if
      call MPI_Gather(nseal, 1, MPI_INTEGER, &
                      nSeaLocal, 1, MPI_INTEGER, &
                      MAPL_Root, comm, status)
      _VERIFY(status)
!
! -------------------------------------------------------------------- /
! Charnock
!
      call MAPL_GetPointer(export, var, 'CHARNOCK', alloc=.true., __RC__)
      if (associated(var)) then
         allocate(svar(nseal), __STAT__)
         call CalcCharnk( svar, __RC__ )
...
!
      ...
      call MAPL_GetPointer(export, var, 'V10M', __RC__)
      if (associated(var)) then
         allocate(svar(nsea), __STAT__)
         svar = UA(1:NSEA) * sin(UD(1:NSEA))
         where(UA(1:NSEA) == UNDEF) svar = MAPL_Undef
         call gsea2grid(svar, var, __RC__)
         deallocate(svar)
      end if
...
!
      call MAPL_GetPointer(export, var, 'DWP', __RC__)
      if (associated(var)) then
         allocate(svar(nseal), __STAT__)
         svar = FP0
         where(svar == UNDEF)
...
!
      call MAPL_GetPointer(export, var, 'TWS', __RC__)
      if (associated(var)) then
         allocate(svar(nseal), __STAT__)
         svar = TWS
         where(svar == UNDEF) svar = MAPL_Undef
...
!
! -------------------------------------------------------------------- /
! Dominant Phase Speed (intrinsic)
!
      call MAPL_GetPointer(export, var, 'DCP0', __RC__)
      if (associated(var)) then
         allocate(svar(nseal), __STAT__)
         svar = FP0
         where(svar == UNDEF)
...
!
! -------------------------------------------------------------------- /
! Dominant Phase Speed: for now same as DCP0
!
      call MAPL_GetPointer(export, var, 'DCP', __RC__)
      if (associated(var)) then
         allocate(svar(nseal), __STAT__)
         svar = FP0
         where(svar == UNDEF)
...
! Mean wave direction
!
      call MAPL_GetPointer(export, var, 'MWD', __RC__)
      if (associated(var)) then
         allocate(svar(nseal), __STAT__)
         svar = THM
         where(svar == UNDEF) svar = MAPL_Undef
...
!
      call MAPL_GetPointer(export, var, 'DWD', __RC__)
      if (associated(var)) then
         allocate(svar(nseal), __STAT__)
         svar = THP0
         where(svar == UNDEF) svar = MAPL_Undef
...
!/
    ...
    contains
      ...
      subroutine sea2grid(svar, gvar, rc)
        ...
        real, allocatable :: glob(:,:), var1d(:)
        integer :: tag, n
        integer :: ix, jx, isea, jsea
        integer :: status
 
        allocate(glob(nx,ny), __STAT__)
        if (amIRoot) then
           glob = MAPL_Undef
           do n = 0, npes-1
              tag = n + 1000
              allocate(var1d(nSeaLocal(n)), __STAT__)
              if (n == MAPL_root) then
                 var1d = svar
              else
                 call MPI_RECV(var1d,nSeaLocal(n),MPI_REAL,n,tag,comm,mpistatus,status)
                 _VERIFY(status)
              end if
              do jsea = 1, nSeaLocal(n)
                 isea = (n+1) + (jsea-1)*npes
                 ix = mapsf(isea,1)
                 jx = mapsf(isea,2)
                 glob(ix,jx) = var1d(jsea)
              end do
              deallocate(var1d)
           end do
        else
           tag = mype + 1000
           call MPI_Send(svar, nseal, MPI_REAL, &
      ...
      subroutine gsea2grid(svar, gvar, rc)
        ...
        do jsea = 1, nSea
           ...
           ix = mapsf(isea,1)
           jx = mapsf(isea,2)
           if  ((ix >= i1) .and. (ix <= in) .and. &
                (jx >= j1) .and. (jx <= jn)) then
              gvar(ix-i1+1,jx-j1+1) = svar(isea)
           end if
        end do
        _RETURN(0)
      end subroutine gsea2grid
...
#define METHOD "CreateImpGrid"
      subroutine CreateImpGrid ( gc, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |        T. J. Campbell, NRL        |
...
!
      ...
      call ESMF_GridCompGet(gc, name=cname, rc=rc)
      if (ESMF_LogFoundError(rc, PASSTHRU)) return
      if (verbosity.gt.0) call ESMF_LogWrite(trim(cname)// &
...
#define METHOD "CreateExpGrid"
      subroutine CreateExpGrid ( gc, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |        T. J. Campbell, NRL        |
...
#define METHOD "CreateImpMesh"
      subroutine CreateImpMesh ( gc, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |     A. J. van der Westhuysen      |
...
#define METHOD "CreateExpMesh"
      subroutine CreateExpMesh ( gc, rc )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |     A. J. van der Westhuysen      |
...
!
      ...
      do k = 0,npet-1
         call ESMF_FieldGather( field,  floc, rootPet=k, vm=vm, __RC__ )
      enddo
...
! -------------------------------------------------------------------- /
!
      if ( natGridIsLocal .OR. .TRUE.) then
...
!	  ix = mapsf(isea,1)
!	  jx = mapsf(isea,2)
!          chk(ix-1+i1,jx-1+j1) = charn(jsea)
          chk(jsea) = charn(jsea)
        enddo jsea_loop
 
      endif !natGridIsLocal
 
!ALT      call ESMF_FieldRedist( chknField, chkField, n2eRH, rc=rc )
!ALT      if (ESMF_LogFoundError(rc, PASSTHRU)) return
 
!ALT      call ESMF_FieldDestroy( chknField, rc=rc )
!ALT      if (ESMF_LogFoundError(rc, PASSTHRU)) return
...
! !ARGUMENTS:
	...
	integer :: status
        ...
        unit = 10 ! ALT: decided to not use MAPL routine GETFILEUNIT
